...
- **`diakrisis-models/`** — **not a build module.** ML model artifacts plus the standalone training/eval source. It defines the contracts the Spring app must honor at serve time. Nothing here is on the Maven classpath; the `.java` files are reference implementations to be ported into `diakritis/`.

> Naming gotcha: the product/SDD is **"diakrisis"** but the Spring module, root package, and artifact are **"diakritis"** (`com.cy.diakritis`). The original package `com.cy diakritis` was invalid; it became `com.cy.diakritis` (group `com.cy` in `pom.xml`). Don't "fix" the spelling — match whatever a given file already uses.

## Build / run / test

All commands run from `diakritis/` (the only Maven reactor):

```bash
cd diakritis
mvn -o package                                      # compile + test + package (offline)
mvn -o -pl <module> -am test                        # build a module (+ its deps) and run its tests
mvn -o test                                         # full test suite (all modules)
mvn -o -pl decision-service -am test -Dtest='GoldenPathTest'          # single test class
mvn -o -pl decision-service -am test -Dtest='GoldenPathTest#t5b_liquidationKillChain_isHoldWithTypology'  # one method
docker compose --profile demo up -d --build         # run the full stack (incl. demo bank)
```

- **Java 26** (pinned in `diakritis/pom.xml`), **Maven** (parent `pom.xml` + per-module poms), **Spring Boot 4.1.0**, **JUnit 5**.
- `diakrisis-models/training/*.java` are **not** built by Maven. They depend on the **Smile** ML library and were designed to run from a separate Maven `trainer/` module (`mvn -q exec:java -Dmain.class=TrainM1`) that does not exist in this workspace yet. Paths inside `TrainM1.java` (`../python/artifacts/...`, `../models/m1`, `../data/raw/ulb/...`) are relative to that future module, not to this repo.
...
- **`MODEL_CARD.md`** — M1 data/split/metrics/caveats. Time-based 70/15/15 split; isotonic-calibrated; PR-AUC ≈ 0.42 (val fold). Percentile-ranked, weight-capped at 18.
