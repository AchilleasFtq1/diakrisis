 ...
 * BCrypt-hashed passwords) the first time iam-service boots against a fresh DynamoDB. Create-if-missing:
 * a username that already exists is left untouched, so reboots and manual edits are preserved.
 *
 * <p>iam-service owns the identity store, so it is the home of this seeder. The demo customers /
 * approver / ops subjects keep their original usernames, roles and account bindings (the golden-path
 * and live demos depend on these); their password is {@code "demo"}. A dedicated {@code admin}
 * principal ({@link Role#ADMIN}, password {@code "admin"}) backs the admin console.
 */
@Component
public class DemoUserSeeder implements ApplicationRunner {
    ...
    private static final List<SeedUser> SEED_USERS = List.of(
            ...
            // Dedicated ADMIN principal backing the /admin/users console.
